...
For `--all`, issues run in ascending issue-number order and each gets the identical single-issue pipeline below — its own `sandcastle/issue-<number>` branch, worktree, integration worktree, verification, landing, report, and (on failure) recovery record. Sequential is the default (`parallelism` 1); parallel mode caps how many issues are in flight at once — the bound comes from `--parallelism` when given, otherwise the `parallelism` in `settings.json` (integer 1–4, set it with `sandcastle configure --parallelism`). There is no unbounded option. A failing issue never aborts the others: in-flight issues finish, the queue continues, and the run ends with a Vietnamese summary listing which issues landed and which failed (failed issues stay open with their recovery state — the summary only says so when a readable record was actually written). An issue that already has a recovery record is never reimplemented by the queue: it is skipped before any branch, worktree, or agent invocation is created, and the output points at `sandcastle retry <issue-number>` to continue the preserved work; a corrupt record is skipped with manual file-cleanup guidance instead. Integration and landing are serialized across concurrent issues, so parallel runs can never race the target branch.
...
Lists every preserved failed task — one entry per recovery record under `.sandcastle/recovery/`. Each entry shows the issue, the phase it stopped in, when it failed, how often it was retried, the preserved source branch and worktree (including whether they still exist), the bounded-repair attempts already spent, and the first line of the recorded error. Stale signals are marked explicitly — a missing source branch, a branch with no unmerged commits (possibly landed elsewhere), or a worktree that is gone (a `retry` rebuilds it from the branch). When the `target..source` comparison cannot be made — e.g. the recorded target branch was deleted or renamed — the entry says the unmerged count is unknown rather than reporting zero; a run that failed before producing any commit is described as incomplete, not landed or stale. A record Sandcastle cannot parse is reported as corrupt — it is never silently dropped, and since neither `retry` nor `discard` can process it, `status` points at manual removal of the record file instead. With nothing preserved, `status` reports there are no failed tasks.
...
Permanently deletes one preserved failed task: the preserved worktree, the source branch, and the recovery record. It first lists exactly what will be removed — the real count of unmerged commits on the preserved branch when the comparison succeeds, or an explicit "unknown" when the target branch is gone or the comparison fails, so you never confirm a delete under a false "0 commits" reading. Interactive runs ask for confirmation first; a declined confirmation leaves everything untouched. Non-interactive runs (no TTY) require `--yes`, so scripts can never delete preserved work accidentally. The record is deleted last — if removing a worktree or branch fails, the record survives so the task stays visible in `status`. Missing or corrupt records produce a Vietnamese diagnosis and nothing is removed.
