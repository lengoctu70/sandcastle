...
const AGY_MARKER_PATTERN =
  /--input-format|--prompt-interactive|mic-serve|antigravity/i;
/** `agy --version` output is a bare version like `1.2.7`. */
const VERSION_PATTERN = /\d+\.\d+(?:\.\d+)*/;
/** `agy models` output when the CLI is not signed in. */
const SIGN_IN_PATTERN =
  /please sign in|not authenticated|authentication required/i;
...
const discoverAntigravity = (
  ...
  Effect.gen(function* () {
    ...
    const isAntigravity =
      HELP_HEADER_PATTERN.test(helpOutput) &&
      AGY_MARKER_PATTERN.test(helpOutput);
    if (!isAntigravity) {
