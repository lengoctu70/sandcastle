...
const makeFakeExec = (
  ...
  const exec: DiscoveryExec = async (command, args) => {
    ...
    if (handler === undefined) {
      return execResult({ stdout: "", stderr: `unknown command: ${key}`, exitCode: 1 });
    }
...
describe("antigravityDiscoveryAdapter", () => {
  ...
  it("parses the real captured catalog fixture end-to-end", async () => {
    ...
    expect(report.models.map((m) => m.id)).toContain("claude-opus-4-6-thinking");
  ...
  it("reports error on spawn failures and timeouts", async () => {
    ...
    expect((await antigravityDiscoveryAdapter.discover(timeoutExec)).state).toBe(
      "error",
    );
    ...
    expect((await antigravityDiscoveryAdapter.discover(slowCatalog)).state).toBe(
      "error",
    );
  });
