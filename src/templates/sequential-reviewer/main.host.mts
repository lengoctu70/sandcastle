...
//
// Both phases share a single host worktree created via createSandbox(), so the
...
// Copy node_modules from the host into each worktree. Host mode reuses your
// installed dependencies as-is — they are already built for this machine, so
// no install step runs inside the worktree. If the project needs a setup step
// per worktree, add `hooks: { host: { onWorktreeReady: [...] } }` to the
// createSandbox() call — host hooks run on this machine.
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  // Create a single worktree that both the implementer and reviewer share.
  // The agent runs directly on this machine via its existing CLI login —
  // no container is started. The explicit branch persists across phases and
  // keeps the work out of your checkout until you merge it.
